...
### 缩进
- 缩进应为一个制表符，不能用空格代替。并且我们建议将文本编辑器的制表符显示设定为8个字符的宽度。
...
### 空格
- 在双目运算符的两侧应各有一个空格，例如：a = b * 22。

- 在单目运算符和操作数之间没有空格，例如：i++。

- 在一行中间的逗号和分号的后面有空格，例如：int a = 1, b = 0; a += b;。

- 不在行首1的花括号的左侧应有一个空格；如果花括号的右侧不立即换行，且不是逗号或分号，应接着有一个空格。

- 左括号的两侧都没有空格，右括号的左侧没有空格。

- 声明指针时，星号与类型之间有空格，与指针名之间没有空格，例如：char *msg;。
...
### 函数声明
- 如果此函数不接受参数，函数定义时可不声明参数列表；但在单独的非定义声明时，必须声明参数列表为void，例如：
```c
...
- 不要使用逻辑取反运算来判断某个返回0为成功的函数成功返回，例如不要这样：
```c
